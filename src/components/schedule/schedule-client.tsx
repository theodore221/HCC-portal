...
interface ScheduleClientProps {
  data: ScheduleRow[];
  /** e.g. "/admin" or "/staff" — used to build booking detail links */
  basePath: string;
}

export function ScheduleClient({ data, basePath }: ScheduleClientProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        {viewMode === "table" ? (
          <ScheduleTable data={filteredData} basePath={basePath} />
        ) : (
          <ScheduleCards data={filteredData} basePath={basePath} />
        )}
      </div>
    </div>
  );
